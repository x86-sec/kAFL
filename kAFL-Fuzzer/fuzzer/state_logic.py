...
class FuzzingStateLogic:
    ...
    def handle_initial(self, payload, metadata):
        ...
        dump_dir = self.config.argument_values['work_dir'] + "/dump/"
    ...
    def handle_redqueen(self, payload, metadata):
        ...
        if not self.config.argument_values['redqueen']:
            return

        redqueen_start_time = time.time()
        success = self.__perform_redqueen(payload, metadata)
        if not success and self.config.argument_values['funky']:
            success = self.__perform_redqueen(payload, metadata)
            if success:
                logger.warn("Funky: RedQ success in 2nd round (id=%d)" % metadata["id"])
        if not success and self.config.argument_values['funky']:
            success = self.__perform_redqueen(payload, metadata)
            if success:
                logger.warn("Funky: RedQ success in 3nd round (id=%d)" % metadata["id"])
